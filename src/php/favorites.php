...
function updateHeaderCounters() {
    ...
    if (isLogged) {
        // Для авторизованных — запрашиваем данные из БД
        Promise.all([
...
function loadFavorites() {
    ...
    if (isUserLoggedIn()) {
        fetch('/src/php/handlers/get_favorites.php')
            .then(res => res.json())
...
function renderFavorites(favs) {
    ...
    container.innerHTML = favs.map(item => `
        <div class="favorite-item">
            ...
            <div class="favorite-actions">
                <button class="add-to-cart-btn" data-id="${item.id}" data-section="${item.section}">В корзину</button>
    ...
    document.querySelectorAll('.remove-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            removeFromFavorites(id, section);
        });
    });
    ...
    document.querySelectorAll('.add-to-cart-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const id = this.dataset.id;
            const section = this.dataset.section;
            addToCartFromFavorites(id, section);
        });
    });
}

// Удаление из избранного
function removeFromFavorites(id, section) {
    if (isUserLoggedIn()) {
        fetch('/src/php/handlers/remove_from_favorites.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `product_id=${id}&section=${encodeURIComponent(section)}`
        }).then(() => {
            loadFavorites();
            // Обновляем счётчик для авторизованных
        ...
        favorites = favorites.filter(item => !(item.id == id && item.section === section));
...
function addToCartFromFavorites(id, section) {
    if (isUserLoggedIn()) {
        fetch('/src/php/handlers/add_to_cart.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `product_id=${id}&section=${encodeURIComponent(section)}&quantity=1`
        }).then(() => {
        ...
        const favorites = JSON.parse(localStorage.getItem('favorites') || '[]');
        const item = favorites.find(fav => fav.id == id && fav.section === section);
        if (item) {
            let cart = JSON.parse(localStorage.getItem('cart') || '[]');
            const existing = cart.find(p => p.id === id && p.section === section);
            if (existing) {
