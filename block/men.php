...
$productsFile = __DIR__ . '/../data/men.json';
if (!file_exists($productsFile)) {
    // Не умираем, если файл пуст — создаём пустой массив
    $mainProducts = [];
} else {
    $mainProducts = json_decode(file_get_contents($productsFile), true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        die('Ошибка в JSON (men.json): ' . json_last_error_msg());
    }
}
...
if (!file_exists($salesFile)) {
    ...
    $salesProducts = json_decode(file_get_contents($salesFile), true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        die('Ошибка в JSON (sales.json): ' . json_last_error_msg());
    }
}

// Объединяем
$initialProducts = array_merge($mainProducts, $salesProducts);
?>
<!doctype html>
<html lang="ru">
    <head>
    ...
    <div class="filters-header">
        ...
        <button id="mobile-filter-close" class="mobile-filter-close">×</button>
    </div>

    <!-- Тип одежды -->
    <div class="filter-accordion">
        <button class="filter-accordion-header">
            Тип одежды
            <span class="accordion-icon">+</span>
        </button>
        <div class="filter-accordion-content">
            <div class="filter-checkboxes">
                <label><input type="checkbox" value="верхняя одежда"> Верхняя одежда</label>
                <label><input type="checkbox" value="штаны"> Штаны</label>
                <label><input type="checkbox" value="шапка"> Шапка</label>
                <label><input type="checkbox" value="очки"> Очки</label>
                <label><input type="checkbox" value="термо бельё"> Термо бельё</label>
                <label><input type="checkbox" value="ветровки"> Ветровки</label>
                <label><input type="checkbox" value="флисовая одежда"> Флисовая одежда</label>
                <label><input type="checkbox" value="обувь"> Обувь</label>
            </div>
        </div>
    </div>
    ...
    <div class="filter-accordion">
        ...
        <div class="filter-accordion-content">
            <input type="text" placeholder="Введите бренд" id="filter-brand" class="filter-input">
        </div>
    </div>
    ...
    <div class="filter-accordion">
        ...
        <div class="filter-accordion-content">
            <div class="filter-colors">
                <label><input type="radio" name="filter-color" value="черный"> Черный</label>
                <label><input type="radio" name="filter-color" value="белый"> Белый</label>
                <label><input type="radio" name="filter-color" value="серый"> Серый</label>
                <label><input type="radio" name="filter-color" value="синий"> Синий</label>
                <label><input type="radio" name="filter-color" value="желтый"> Желтый</label>
                <label><input type="radio" name="filter-color" value="оранжевый"> Оранжевый</label>
                <label><input type="radio" name="filter-color" value="камуфляж"> Камуфляжный</label>
                <label><input type="radio" name="filter-color" value="красный"> Красный</label>
                <label><input type="radio" name="filter-color" value="зеленый"> Зеленый</label>
                <label><input type="radio" name="filter-color" value="фиолетовый"> Фиолетовый</label>
                <label><input type="radio" name="filter-color" value="коричневый"> Коричневый</label>
            </div>
        </div>
    </div>
    ...
    <div class="filter-accordion">
        ...
        <div class="filter-accordion-content">
            <div class="filter-price">
                <input type="number" id="filter-min" min="0" value="0" class="filter-price-input">
                <span>—</span>
                <input type="number" id="filter-max" min="0" value="99999" class="filter-price-input">
            </div>
        </div>
    </div>

    <button id="apply-filters" class="filter-apply-btn">ПРИМЕНИТЬ</button>
</aside>

        <main class="catalog-content" >
            <div class="catalog-header">
                <h1 class="catalog-title">Товары</h1>
                <div class="products-count" id="products-count">Найдено: <span id="count-number">0</span> товаров</div>
            ...
            <a href="#upPage" class="upPage" id="upPageBtn">
                <img src="../public/icon/upPage.png" alt="Наверх">
            </a>
        </main>
    ...
    <script>
        const initialProducts = <?= json_encode($initialProducts, JSON_UNESCAPED_UNICODE) ?>;
    </script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // По умолчанию открываем "Тип одежды" и "Цена"
            if (accordions.length > 0) {
                accordions[0].classList.add('active'); // Тип одежды
                accordions[accordions.length - 1].classList.add('active'); // Цена
            }
        });
    </script>
