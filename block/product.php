...
// Правильный путь: из /block/ → ../data/
$filepath = __DIR__ . "/../data/{$section}.json";

if (!file_exists($filepath)) {
    http_response_code(404);
    die("Категория '{$section}' не найдена. Путь: " . $filepath);
}

$products = json_decode(file_get_contents($filepath), true);

if (json_last_error() !== JSON_ERROR_NONE) {
    die("Ошибка чтения JSON: " . json_last_error_msg());
}

$product = null;

foreach ($products as $p) {
    // Сравниваем по id (без пробелов — в твоём JSON их нет)
...
if (!$product) {
    http_response_code(404);
    die("Товар с id={$id} не найден в {$section}");
}
...
<head>
    ...
    <title><?= htmlspecialchars($product['title']) ?> — PEAK</title>
...
<body>
    ...
    <div class="product-page">
        <div class="product-gallery">
            <img src="<?= htmlspecialchars($product['images'][0]) ?>" alt="<?= htmlspecialchars($product['title']) ?>" class="main-image" id="mainImage">
            <?php if (count($product['images']) > 1): ?>
                <button class="gallery-nav prev" id="prevImg"><img src="../public/icon/arrow-small.png" alt="назад"></button>
                <button class="gallery-nav next" id="nextImg"><img src="../public/icon/arrow-small.png" alt="назад"></button>
            <?php endif; ?>
            <div class="thumbnails">
                <?php foreach ($product['images'] as $index => $img): ?>
                    <img src="<?= htmlspecialchars($img) ?>" alt="" class="thumbnail <?= $index === 0 ? 'active' : '' ?>" data-src="<?= htmlspecialchars($img) ?>">
                <?php endforeach; ?>
            </div>
        </div>

        <div class="product-info_page">
            <h1 class="product-brand-page"><?= htmlspecialchars($product['brand']) ?></h1>
            <div class="product-title-page"><?= htmlspecialchars($product['title']) ?></div>
            <div class="valid_info-page">
                <div class="product-color-page">
                    <strong>Цвет:</strong>
                    <span><?= htmlspecialchars($product['color']) ?></span>
                </div>
                <?php if (!empty($product['article'])): ?>
                    <div class="product-article-page">
                        Артикул: <span><?= htmlspecialchars($product['article']) ?></span>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="size-selector">
                ...
                <input type="hidden" id="selectedSize" value="<?= htmlspecialchars($availableSizes[0]) ?>">
            </div>
            
            <div class="info-price">Купить <?= htmlspecialchars($product['title']) ?> можно за:</div>
            <div class="product-price-page"><?= number_format($product['price'], 0, ',', ' ') ?> ₽</div>

            <div class="product-actions">
                <button class="btn-add" id="addToCart"
                    data-id="<?= (int)$product['id'] ?>"
                    data-section="<?= htmlspecialchars($product['section']) ?>">В корзину</button>
                <button class="btn-fav" id="addToFav"
                    data-id="<?= (int)$product['id'] ?>"
                    data-section="<?= htmlspecialchars($product['section']) ?>">В избранное</button>
            </div>
        </div>
    </div>
...
if (prevBtn && nextBtn) {
    ...
    document.getElementById('addToCart').addEventListener('click', function() {
        ...
        let selectedSize = hiddenSize.value;

        if (selectedSizeEl) {
            selectedSize = selectedSizeEl.dataset.size;
        }
        ...
        const item = {
            ...
            article: <?= json_encode($product['article'] ?? 'N/A') ?>
        };
        ...
        if (isUserLoggedIn()) {
            fetch('/src/php/handlers/add_to_cart.php', {
                ...
                body: `product_id=${item.id}&section=${encodeURIComponent(item.section)}&quantity=1&size=${encodeURIComponent(selectedSize)}`
            }).then(() => {
    ...
    document.getElementById('addToFav').addEventListener('click', function() {
        ...
        if (isUserLoggedIn()) {
            ...
            const exists = favs.some(p => p.id == item.id && p.section === item.section);
            if (!exists) {
                favs.push(item);
